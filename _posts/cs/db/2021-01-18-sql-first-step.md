---
title: "[SQL 첫걸음] 요약 정리"
excerpt: "SQL 첫걸음(아사이 아츠시 저)를 읽고 요약 정리한다."
categories:
  - Database
tags:
  - Database
...
개인적으로 중요하다고 생각하는 부분들만 요약 정리했습니다.
...
* 기능성.
  * 대용량 및 대규모 데이터 처리 요청하는 기능을 제공하며, 유저가 관리 기능을 확장할 수 있어 시스템 개발이 유연해진다.
* 신뢰성.
...
표 형식의 데이터는 행(레코드)와 열(컬럼/필드)로 구성된다. 행과 열이 만나는 지점을 셀이라고 하며, 하나의 데이터 값이 저장된다. 각 데이터는 각 열은 일반 프로그래밍 언어들처럼 자료형을 가진다.
...
* ``SELECT * FROM 테이블명 ORDER BY 열1 ASC, 열2 DESC`` 처럼 복수의 열을 정렬 기준에 포함할 수 있다.
  * ORDER BY로 지정된 열에 NULL 값이 있는 행은 가장 처음 혹은 가장 마지막에 표시된다.
...
* ``DELETE FROM 테이블명 WHERE 조건식`` 으로 한 행을 삭제할 수 있다.
* ``UPDATE 테이블명 SET 열1 = 값1, 열2 = 값2, ... WHERE 조건식``으로 특정 행에 데이터 갱신이 가능하다.
  * ``UPDATE 테이블명 SET 열1 = 열1 + 1``과 같이 테이블의 모든 행에 대해 하나의 열 값을 연산할 수 있다.
...
* 스키마 설계 : DDL을 통해 DB에 테이블을 작성해서 구축해나가는 작업이다.
...
* ``ALTER TABLE 테이블명 [기존 열 이름] [신규 열 정의]``
...
* 유일성 제약 : 기본키로 설정된 열이 중복하는 데이터 값을 가질 수 없다.
