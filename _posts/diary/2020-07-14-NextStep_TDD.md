...
Java와 Spring을 중심으로 개발 공부를 시작했지만, 코드 리뷰를 받을 기회가 없었다. 객체, 인터페이스, 다형성 등 개념은 알지만 이를 적절히 활용하고 있는지 알 수 없었다. 또 내가 봐도 이해가 안 될만큼 코드들이 지저분했다.

2020년 1월부터 42 Seoul이라고 하는 SW 교육 프로그램에 참여하면서, 학생들끼리 코드를 리뷰하게 되었지만 문제점이 많았다.

* Java 코드 리뷰가 아닌 C 코드 리뷰.
* 리뷰어에 따른 피드백의 수준 차이.
  * 나를 포함하여 실무 경험이 없는 비전공자 학생들이 많다.

교육 때문에 C를 하고 있지만, 곧 다시 Spring을 공부할 것이기 때문에 그 전에 Java의 OOP와 클린 코드에 대해 공부를 하고 싶었다.

우아한테크코스를 수료했던 고등학교 동창이 **NextStep의 TDD, Refactoring, Clean Code With Java** 수업을 추천해주었다. 어디가서 듣기 어려운 고 퀄리티의 수업이며, 현재 내가 가진 니즈를 충족시켜줄 것이라고 했다.
...
이때 나도 42 Seoul이 코로나로 인해 온라인 비대면 학습으로 전환됨에 따라, 시간적인 여유가 충분해져서 고민없이 수강할 수 있게 되었다.
...
2. 매 과제 별로 개발자 역량에 대한 교양 영상과, TDD, Refactoring, Clean Code의 개념 및 방법을 설명하는 영상이 제공된다.
...
리뷰어는 직접적인 정답을 제시해주기 보다는, 방향성에 대한 흰트를 제시해주신다. 또한 띄어쓰기 같은 코드 컨벤션까지 섬세하게 지적해주셔서 당황스러웠다.
...
NextStep 홈페이지에는 수강 대상이 **당신이 실무 경력 최소 1~3년 이상의 자바 개발자이며** 라고 명시되어있다. Slack에 있는 수강생들 중 많은 분들이 현직 백엔드 개발자처럼 보이기도 하고.

막상 등록하니 실무 경력 1도 없고, Java 조금 쓸 줄 아는 학생이 들어도 되나 살짝 겁났다. 하지만 시간적인 여유가 충분하다면 실무 경력이 없더라도 Spring을 공부하는 학생이 듣기에 좋은 수업이라고 생각한다.
...
처음 과제를 하는데 되게 스트레스를 받았다. 기능 구현이 어려워서가 아니라, 내 코드가 OOP스럽지 않다는 것을 느꼈기 때문이다. 최근 C만 써서 그런지는 몰라도, 코드들이 너무 절차지향적이었다.
...
이런 방식으로 코드를 짜는게 처음에는 어색했지만, 객체의 책임을 분명하게 나누는 연습을 하고 피드백을 지속적으로 받았다. 덕분에 평소 잘 활용하지 못했던 Lambda, Stream과 더불어서 Enum, Interface 등을 통해 Java와 OOP에 더욱 친숙해질 수 있었다.

* TDD, Refactoring

나는 도메인을 구현할 때, 처음부터 완벽하게 설계하려고 노력을 많이했었다. 도메인을 가장 작은 단위의 객체로 나누어 아래서부터 구현을 시작했다. 요구사항이 간단한 과제는 가능했지만, 도메인 지식이 부족한 과제에서는 큰 어려움으로 다가왔다. 객체 추상화도 어렵고, 어찌저찌 객체 설계를 완료하고 기능을 구현하다 보면 코드가 엉켜버린다.
...
완벽한 설계에 집착하고, 코드를 여러번 갈아 엎으면서 나 또한 도메인 지식이 쌓이기 시작하고 작은 단위의 객체 개발이 가능해졌다. 하지만 결정적인 순간에 지친 나머지 리팩토링 힌트를 참고했던 점들이 아쉬움으로 남는다. 조금만 더 에너지를 아껴가며 과제를 수행했다면, 힌트를 참조하지 않고 스스로 객체 분리 리팩토링에 성공했을텐데.
...
나는 이런 선택이 나쁘다고 말하는 것이 아니다. 21세기 대한민국 사회에서, 특히 문과 학생에게는 가장 현실적인 방법이기 때문이다. 항상 내가 하고 싶은 일만 하면서 살 수는 없는 노릇이다. 내가 배부른 소리를 하는 것일 수도 있다. 하지만 나는 조금 더 시간이 걸리더라도, 내가 하고 싶은 일을 찾고 싶었다.

그리고 개발 공부를 시작했다. 예전에 빅데이터 분석 수업을 들으면서 배운 컴퓨터 언어가 즐거웠던 기억으로 부터 시작한 도전이었다. 개발 공부가 항상 즐거울 수만은 없다. 업으로 삼게 되면, 내가 좋아하는 개발보다 귀찮고 어렵고 짜증나는 개발을 많이 하게 될 것이다. 그럼에도 불구하고 내가 열정과 흥미를 가질 수 있는 분야의 직무를 선택했다는 점에서 항상 즐겁다.

개발 공부를 시작한지 1년이 되었고, 아직 부족한 점이 많다. 항상 더 나은 사람으로 발전하기 위해 노력해야겠다. NextStep의 TDD, Refactoring, Clean Code With Java 과정 마지막 수업의 내용을 마지막으로 이 글을 마친다.
