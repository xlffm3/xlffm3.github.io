...
2020년 1월부터 42 Seoul이라고 하는 SW 교육 프로그램에 참여하면서, 학생들끼리 코드를 리뷰하게 되었지만 다소 부족한 점이 있었다.

* Java 코드 리뷰가 아닌 C 코드 리뷰.
  * 코드의 품질에 대한 리뷰보다는 과제 기능 구현에 대한 리뷰가 중심이었다.
* 리뷰어에 따른 피드백의 수준 차이.
  * 나를 포함하여 실무 경험이 없는 비전공자 학생들이 많다.

교육 때문에 C를 하고 있지만, 곧 다시 Spring을 공부할 것이기 때문에 그 전에 Java의 OOP와 클린 코드에 대해 공부를 하고 싶었다.

정신없이 C를 공부하고 있던 때, 고등학교 동창이 **NextStep의 TDD, Refactoring, Clean Code With Java** 수업을 추천해주었다. 어디가서 듣기 어려운 고 퀄리티의 수업이며, 현재 내가 가진 니즈를 충족시켜줄 것이라고 했다.
...
리뷰어는 직접적인 정답을 제시해주기 보다는, 방향성에 대한 흰트를 제시해주신다. 띄어쓰기 같은 사소한 컨벤션 등 코드에 대해 정말 섬세하게 지적해주셔서 놀랐다. 퇴근하고 미션을 수행하는 직장인 수강생들도 힘들었겠지만, 퇴근하고 새벽까지 피드백을 남겨주시는 리뷰어님들의 체력에 다시 한번 감탄.
...
개발 공부를 시작한지 1년이 되었고, 아직 부족한 점이 많다. 항상 어제보다 더 나은 사람으로 발전하기 위해 노력해야겠다. NextStep의 TDD, Refactoring, Clean Code With Java 과정 마지막 수업의 내용을 마지막으로 이 글을 마친다.
