...
이런 환경 속에서 성장하기 위해서는, 끊임없이 자신의 부족한 점을 진단하고 필요한 지식을 자기주도적으로 학습해야 합니다. 저 또한 더 나은 품질의 어플리케이션 설계를 위해, 관련 서적(클린 코드, 리팩토링, 디자인 패턴, 아키텍쳐)을 참고하고 열심히 구글링하면서 학습했습니다. 아울러 PullRequest를 통해 다른 참가자들의 코드를 보면서, 다양한 문제 접근 및 해결 방식을 배울 수 있었습니다.

<br>

## 3. 클린 코드를 위해

프리코스 미션에서 가장 많은 시간을 할애한 것은 네이밍 리팩토링 과정이었습니다. 기능을 다 구현하고 나서도, 변수와 메서드 및 클래스의 이름이 직관적인지를 항상 고민했습니다.

이를 위해 협업을 위한 Java 네이밍 컨벤션 아티클을 주로 참고했으며, 미션의 PullRequest를 통해 다른 사람들의 다양한 네이밍 방식을 최대한 많이 경험하려고 노력했습니다.
